...
class AuctionHouse extends Component {
  ...
  verify = (auction_id, type) => async (e) => {
    ...
    try {
      ...
      console.log(err);
      alert(`Error in decrypting key:${err}`);
    }
  };
  ...
  confirm = (auction_id, type) => async (e) => {
    ...
    try {
      if (type === "Normal Listing") {
        await market.methods.confirmDelivery(auction_id)
            .send({ from: currentAccount,gas: 1000000,gasPrice: this.state.web3.utils.toWei('20', 'gwei') });
      } else if (type === "Blind Auction") {
        await blind_contract.methods.confirmDelivery(auction_id).send({ from: currentAccount,gas: 1000000,gasPrice: this.state.web3.utils.toWei('20', 'gwei') });
      } else if (type === "Vikrey Auction") {
        await vickrey_contract.methods.confirmDelivery(auction_id).send({ from: currentAccount,gas: 1000000,gasPrice: this.state.web3.utils.toWei('20', 'gwei') });
      } else {
        await average_contract.methods.confirmDelivery(auction_id).send({ from: currentAccount,gas: 1000000,gasPrice: this.state.web3.utils.toWei('20', 'gwei') });
      }
    } catch (error) {
    ...
    try {
      if (type === "Normal Listing") {
        ...
        await this.state.market.methods.requestBuy(auction_id, publickey)
          .send({
            from: currentAccount,
            value: web3.utils.toWei(String(price * 2), "ether")
          });
      } else {
